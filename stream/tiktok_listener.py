...
TikTok Live comment listener — uses TikTokLive library.
Real-time comments, gifts, follows from TikTok LIVE WebCast service.
No TikTok developer account needed.

Install:  pip install TikTokLive
Optional: Euler Stream key (eulerstream.com) for production reliability.
"""
...
class TikTokListener:
    """
    Connects to a TikTok LIVE stream and forwards every event
    to the Synthcast agent API.

    Events handled:
      comment   → /comment/process
      gift      → /comment/process  (with gift_value)
      follow    → /comment/process  (with is_new_follower=True)
      join      → /comment/process  (20% sample to avoid spam)
      subscribe → /comment/process  (with is_subscriber=True)
    """
    ...
    def __init__(
        ...
        self.euler_api_key  = euler_api_key or os.getenv("EULER_API_KEY")
        ...
        if simulation_mode:
            print("[TikTok] SIMULATION MODE — fake comments only")
        else:
            print(f"[TikTok] Ready for @{self.unique_id}")
            status = "Euler Stream key loaded" if self.euler_api_key else "No Euler key (dev mode OK)"
    ...
    async def _connect_live(self):
        ...
        async def on_gift(e: GiftEvent):
            # Skip intermediate streak events
        ...
        @tl.on(JoinEvent)
        async def on_join(e: JoinEvent):
            # Sample 20% of joins — busy streams get hundreds per minute
        ...
        @tl.on(SubscribeEvent)
        async def on_subscribe(e: SubscribeEvent):
            await self._forward({
                "platform":        "tiktok",
                "username":        e.user.nickname or e.user.unique_id,
        ...
        print(f"[TikTok] Connecting... make sure @{self.unique_id} is LIVE on TikTok")
        try:
            await tl.start()
        except Exception as e:
            msg = str(e).lower()
            if "not live" in msg or "not found" in msg:
                ...
                print("Add EULER_API_KEY to your .env — get one free at eulerstream.com")
            else:
                print(f"\nConnection error: {e}")
    ...
    async def _simulate(self):
        EVENTS = [
            ...
            {"username": "nightowl_live",   "text": "Love this stream! Amazing content", "gift_value": 0,    "is_new_follower": False},
            ...
            {"username": "loyal_fan_01",    "text": "you are the GOAT no question",      "gift_value": 0,    "is_new_follower": False},
            {"username": "questioner44",    "text": "do you have merch?",                "gift_value": 0,    "is_new_follower": False},
            ...
            {"username": "spam_bot_99",     "text": "follow me follow back!!!",           "gift_value": 0,    "is_new_follower": False},
            ...
            {"username": "hype_lord",       "text": "LETS GOOOOO",                       "gift_value": 0,    "is_new_follower": False},
        ]
    ...
    async def _forward(self, payload: dict):
        ...
        try:
            ...
            if result:
                print(f"  @{u}: {t}")
                print(f"  → [{result.get('tone','?')}] {result['text'][:70]}  (~{result.get('estimated_duration_s',0)}s)")
            else:
                print(f"  [SKIP] @{u}: {t}")
            ...
            self._errors += 1
            if self._errors <= 2:
                print(f"[TikTok] Cannot reach {self.agent_url} — is the API server running?")
                print("  Run: uvicorn api.main:app --reload --port 8000")
        except httpx.HTTPStatusError as e:
            self._errors += 1
            if e.response.status_code == 400:
                print(f"  [NO SESSION] Call /session/start first")
        except Exception as e:
            self._errors += 1
            if self._errors <= 5:
                print(f"  [ERROR] {e}")
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
            ...
            print("Start the server: uvicorn api.main:app --reload --port 8000")
            return
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                print("\nSession stats:", json.dumps(resp.json(), indent=2))
            except Exception:
                pass
